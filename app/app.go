...
func serveError(ctx appengine.Context, w http.ResponseWriter, err error) {
...
func handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	if len(other["name"]) != 1 || len(other["version"]) != 1 || len(other["os"]) != 1 || len(other["arch"]) != 1 || len(other["signature"]) != 1 {
		serveError(ctx, w, fmt.Errorf("must specify all of name, version, os, arch and signature"))
		return
	}
	if len(blobs) != 1 || len(blobs["file"]) != 1 {
		log.Printf("Got blobs: %v", blobs)
		// log.Printf("Got others: %v", other)
		serveError(ctx, w, fmt.Errorf("must specify exactly one upload file named 'file' as the container to upload"))
		return
	}
	signature, err := ioutil.ReadAll(base64.NewDecoder(base64.URLEncoding, bytes.NewBuffer([]byte(other["signature"][0]))))
	if err != nil {
		serveError(ctx, w, fmt.Errorf("got an error while trying to read signature: %v", err))
		return
	}

	// Check for an existing registry, create it if necessary.
	rkey := registryKey(ctx, other["name"][0])
	var registry ContainerRegistry
	err = datastore.Get(ctx, rkey, &registry)
	if err != nil && err != datastore.ErrNoSuchEntity {
		serveError(ctx, w, fmt.Errorf("unable to find registry: %v", err))
		return
	}
	if err == datastore.ErrNoSuchEntity {
		if _, err := datastore.Put(ctx, rkey, &ContainerRegistry{Name: other["name"][0]}); err != nil {
			serveError(ctx, w, fmt.Errorf("failed to create container registry: %v", err))
		}
	}
	...
	keys, err := datastore.NewQuery("Container").
		...
		GetAll(ctx, &existing)
	if err != nil {
		serveError(ctx, w, fmt.Errorf("unable to query registry: %v", err))
	}
	for i := range existing {
		if err := blobstore.Delete(ctx, existing[i].BlobKey); err != nil {
			serveError(ctx, w, fmt.Errorf("unable to delete existing container: %v", err))
		}
		if err := datastore.Delete(ctx, keys[i]); err != nil {
			serveError(ctx, w, fmt.Errorf("unable to remove existing container: %v", err))
		}
	}

	// Put the container into the datastore
	key := datastore.NewIncompleteKey(ctx, "Container", rkey)
	entry := &Container{
...
func registryKey(ctx appengine.Context, name string) *datastore.Key {
	key := datastore.NewKey(ctx, "ContainerRegistry", name, 0, nil)
	if key == nil {
...
func handleRoot(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) < 7 {
		serveError(ctx, w, fmt.Errorf("invalid request"))
		return
	}
	name := parts[2]
	version := parts[3]
	...
	var registry ContainerRegistry
	rkey := registryKey(ctx, name)
	if err := datastore.Get(ctx, rkey, &registry); err != nil {
		serveError(ctx, w, fmt.Errorf("unable to get reigstry %q: %v", name, err))
		return
	}
	it := datastore.NewQuery("Container").Ancestor(rkey).Run(ctx)
	var newest string
	...
	if !found {
		serveError(ctx, w, fmt.Errorf("failed to find container"))
		return
	}
	if asc {
